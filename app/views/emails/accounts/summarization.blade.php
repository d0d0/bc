...
<html lang="en-US">
    ...
    <body>
        <h2>Prehľad stavov účtov ku dňu {{Carbon::now()->format('d.m.Y') }}</h2>
        <table class="table table-hover table-condensed">
            <thead>
                <tr>
                    ...
                    <th>.Priezvisko</th>
                    <th>.Reálny účet</th>
                    <th>.Imaginárny účet</th>
                </tr>
            </thead>
            <tbody>
                @foreach(User::orderBy('surname', 'ASC')->orderBy('name', 'ASC')->get() as $user)
                <tr class="{{ ($user->imaginaryAccount->amount < 0 || $user->realAccount->amount < 0) ? 'danger' : '' }}">
                    <td>{{{ $user->name }}}</td>
                    <td>{{ $user->surname }}</td>
                    <td>{{{ round($user->realAccount->amount, 2) }}}</td>
                    <td>{{{ round($user->imaginaryAccount->amount, 2) }}}</td>
                </tr>
                @endforeach
            </tbody>
        </table>
